...
class LessThan:
    ...
    def __str__(self):
        return "Exp LessThan: [{}] < [{}]".format(self.left, self.right)
...
class While:
    ...
    def __str__(self):
        return "While: condition[{}], body[{}]".format(str(self.condition), 
            str(self.body))
...
print("Verify statement {Sequence: x = 1 + 1， y = x + 3}, {env: {}}")
Machine(
...
print("Verify statement {While :  x < 20 ？x = x * 2}, {env: {x := 2}} ")
Machine(
    While(LessThan(Variable('x'), Number(20)), Assign('x', Multiply(Variable('x'), Number(2)))),
    Env({'x':Number(2)})).run()
